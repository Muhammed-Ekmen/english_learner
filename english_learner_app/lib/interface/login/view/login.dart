...
import 'package:english_learner_app/widgets/buttons/social_media_button.dart';
...
import 'package:english_learner_app/interface/login/view_model/login_view_model.dart';
...
class LoginScreen extends StatelessWidget with IShaderMask {
  ...
  @override
  Widget build(BuildContext context) => WillPopScope(onWillPop: WillPopBehaviour.alert.apply, child: Scaffold(body: customShaderMask(shaderBody: Padding(padding: ConstSizes.screenHorizantal.apply, child: CustomScrollView(slivers: [sliverFillRemain])))));
  ...
  get sliverFillRemain => SliverFillRemaining(
        ...
        child: Column(
          ...
          children: [
            Text(ConstTexts.intance.welcomeMessage, style: TextStyle(color: CustomColor.athensGray.apply, fontWeight: FontWeight.w900, fontStyle: FontStyle.italic, fontSize: 24)),
            englishLearnerApp,
            ...
            socialMediaButtons,
            dividerAndOr,
            RegularButton(onTap: _loginViewModel.createAnAccount, title: ConstTexts.intance.createAnAccount.toUpperCase(), backGorundColor: CustomColor.rossoCorsa.apply),
          ],
        ),
      );
  ...
  get englishLearnerApp => FittedBox(
        child: AnimatedTextKit(
          repeatForever: true,
          animatedTexts: [
            ColorizeAnimatedText(ConstTexts.intance.appTitle, textStyle: ConstTextStyles.title.apply, colors: [CustomColor.athensGray.apply, Colors.transparent])
          ],
        ),
      );
  ...
  get dividerAndOr => Row(children: [Expanded(flex: 6, child: divider), Expanded(flex: 2, child: Text(ConstTexts.intance.or, style: ConstTextStyles.content.apply, textAlign: TextAlign.center)), Expanded(flex: 6, child: divider)]);

  get divider => Divider(height: 0, color: CustomColor.athensGray.apply, thickness: 3);
}
